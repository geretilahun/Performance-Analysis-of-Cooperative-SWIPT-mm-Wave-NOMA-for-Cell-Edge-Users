...
for ss = 1:length(SNR_dB)
    ...
    for ii = 1:G
        ...
        for zz = 1:Sim_times
           if (snr_SN_xF(zz) < rho2h)
            OP_N_sim_temp = OP_N_sim_temp + 1;
        elseif (snr_SN_xF(zz) >= rho2h) && (snr_SN_xN(zz) < rho1h)
            OP_N_sim_temp = OP_N_sim_temp + 1;
          end 
        end
        OP_N_sim(ss) = OP_N_sim_temp/Sim_times;
        %count1 = snr_SN_xF < rho2h;
        %count2 = snr_SN_xF >=rho2h & snr_SN_xN < rho1h;
        %OP_N_sim(ss) = sum(count1+count2)/Sim_times;
        %% Analysis
        ...
        theta = pF/pN;
        %
        if rho2h < theta && mu_a >= rho1h/a2
            OP_N_ana(ss) = 1 - exp(-mu_a/lSN);
        elseif rho2h < theta && mu_a < rho1h/a2
            OP_N_ana(ss) = 1 - exp(-rho1h/lSN/a2);
        elseif rho2h >= theta
            OP_N_ana(ss) = 1;
        end
    end
...
%% plot
semilogy(SNR_dB,OP_N_sim,'o:',...
    SNR_dB,OP_N_ana,'*-')
legend('Location','southwest','User N SWIPT-CNOMA (sim.)', 'User N CNOMA (ana.)')
%
xlabel('SNR (dB)')
ylabel('OP')
%
save data_OP_N_sim.dat OP_N_sim -ascii
save data_OP_N_ana.dat OP_N_ana -ascii
