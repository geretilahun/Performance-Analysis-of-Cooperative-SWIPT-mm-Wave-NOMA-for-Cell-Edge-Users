        ...
        %
         term1 = 0;
            for ii=0:1:G
                for jj=(G-ii):-1:0
                    kk = G - (ii+jj);
                    A1 = factorial(G)/factorial(ii)/factorial(jj)/factorial(kk);
...
%                     A5 = integral(fun1,0,inf);
                    ...
                    A5 = sqrt(4*(jj+kk)*rho2h /lSN/c*lNF)...
                        *besselk(1,sqrt(4*(jj+kk)*rho2h /lSN/c/lNF));
                    end
                    A4 = A5 - Integral_Aprx(rho2h /c/mu_a,1/lNF,(jj+kk)*rho2h /lSN/c);
                    %
                    term1 = term1 + (A1 * A2 * A3 * A4);
                end
            end
            term2 = ((1- exp(-mu_a/lSN)).^G)*exp(-rho2h /lNF/c/mu_a);
            OP_F_ana(ss) =  term1+term2;
     end
end
%% plot
